...
def _construir_prompt(datos: dict, days: int) -> tuple[str, str]:
    ...
    system = (
        ...
        "- Si el usuario tiene hipoglucemias post-ejercicio, NUNCA sugieras bolos de insulina "
        "antes o durante el ejercicio como solución. El ejercicio aeróbico baja la glucosa; "
        "más insulina la bajaría aún más. La solución siempre es carbohidratos sin insulina, "
        "reducir el bolo previo, o ajustar la basal con el médico.\n"
        "- Si la glucosa está cayendo (ROC negativo, hipo activa), nunca sugieras insulina.\n"
        "- Si detectás hipos recurrentes, la causa siempre es exceso de insulina o falta de "
        "carbohidratos — nunca déficit de insulina.\n"
        "- Distinguí claramente entre hipoglucemia (tratar con carbos rápidos) e "
        "hiperglucemia (ahí sí puede corresponder insulina de corrección, con médico).\n"
        "- Ante la duda sobre una recomendación de insulina, omitila y pedí consulta médica."
    )
